...
Surface the tool — tool_search("FMP quote") once per session to load FMP:quote (deferred MCP tool).
Per-ticker single quote — FMP:quote(endpoint="quote", symbol="TK") for each ticker. Returns price · change · changePercentage · day low/high · 52wk low/high · marketCap · previousClose · timestamp.
LOOP, never batch. batch-quote and every sibling batch/aftermarket endpoint are PREMIUM-GATED → ACCESS DENIED on Starter. ~12 single calls = the whole portfolio. Do not retry batch in-session.
Tickers from FINN_STATE.json positions (portfolio) or the named set (live watch / dip check / single ticker / report). Never hardcode a price.
