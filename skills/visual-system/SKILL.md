...
Button — primary (solid violet, white, sentence sans) / secondary (tinted ghost) / tertiary (grey ghost) / status pill (semantic-dim). ONE primary per view. Command buttons lowercase mono (acronyms UPPER mono); CTAs sentence sans.
Input — recessed field (bg2) + attached primary (Go). Live suggestion dropdown filters commands + tickers; Enter fires highlighted; selected row = active token.
Tile — icon-above-label (Tabler 22) + lowercase-mono command label + optional desc + feed-driven corner badge (live/count/alert). Auto-fit grid. The only component that lifts.
Row — leading slot (icon-box 32 / status emoji / small icon) + body + trailing slot (chevron = navigates / CTA = acts / pill = status / none). Command rows lowercase mono; action rows prose sentence sans. No lift.
Chip — interactive conviction-tinted clickable token (c5 violet / c4 info / c3 warn), optional trailing live value; + static Tag variant (nano, semantic-dim fill, non-interactive: WIN / DIP / EARN / CONFIRMED / FINN PROJECTION). Tickers UPPER mono.
Segmented control — single-select, one always sticky-on; on = active token. Sans lowercase option labels. No lift. (route toggle, value/% toggle.)
Callout — semantic emphasis in three forms: chip (inline fact) / banner (full-width context one-liner) / box (header + multi-line). Color = severity (neg alert / warn caution / pos opportunity / info note / violet accent-verdict). Restraint — routine status uses Tags, lists use Rows.
Stat tile — display only, non-interactive. Variants hero (display 25) / progress (4px feed-driven bar) / delta (sub in semantic color). Status strip = connected grid, 1px hairline dividers. Numbers always mono.
